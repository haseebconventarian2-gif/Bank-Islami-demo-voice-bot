...
def detect_language(text: str) -> str:
    return "ur" if _contains_arabic_script(text) else "en"


def select_voice(lang: str) -> str:
    if lang == "ur":
        return os.getenv("AZURE_TTS_VOICE_URDU") or os.getenv("AZURE_TTS_VOICE", "ur-PK-AsadNeural")
    return os.getenv("AZURE_TTS_VOICE_EN") or os.getenv("AZURE_TTS_VOICE", "alloy")


URDU_GREETINGS_ROMAN = {
    "salam",
    "salaam",
    "assalamualaikum",
    "asalamualikum",
...
def is_urdu_greeting(text: str) -> bool:
    lowered = text.lower()
    if any(token in lowered for token in URDU_GREETINGS_ROMAN):
        return True
    if any(token in text for token in URDU_GREETINGS_URDU):
        return True
    return False
...
def is_english_greeting(text: str) -> bool:
    lowered = text.lower()
    return any(token in lowered for token in ENGLISH_GREETINGS)


def replace_hard_words(text: str) -> str:
    updated = text
    for src, dst in HARD_WORD_REPLACEMENTS.items():
...
def create_app() -> FastAPI:
    ...
    async def translate_to_english(text: str) -> str:
        ...
        translated = await generate_text(user_prompt=text, system_prompt=system, use_tools=False)
        return (translated or "").strip()

    async def process_query_with_lang(user_text: str) -> tuple[str, str]:
        """
        ...
        Returns:
            Response text and detected language
        """
        if not user_text or not user_text.strip():
            return "Please provide a message or question.", "en"

        # Handle greetings
        normalized = user_text.strip().lower()
        if normalized in {"hi", "hello", "hey", "salam", "assalamualaikum", "asalamualaikum"}:
            return "Assalam-o-Alaikum! Welcome to Bank Islami. How can I help you today?", "en"

        lang = detect_language(user_text)
        print(f"Detected language: {lang}")
        search_query = user_text
        if lang == "ur":
            search_query = await translate_to_english(user_text)
            print(f"Translated query: {search_query}")
        
        # Build RAG context from Azure AI Search
        rag_context = await build_rag_context(search_query)
        print(f"RAG context length: {len(rag_context or '')}")
        
        # Generate response using GPT-4o with RAG context
        try:
            reply_language = "Urdu" if lang == "ur" else "English"
            rag_system_prompt = (
                f"{system_prompt}\n\n"
                "Use ONLY the context provided. If the answer is not in the context, "
                "say you don't know.\n"
                f"Reply in {reply_language}. If replying in Urdu, use simple, everyday words that are easy to pronounce. Avoid the word 'buniyad'."
            )
            ...
            return response or "Sorry, I could not generate a response.", lang
        except Exception as e:
            print(f"Error generating response: {e}")
            return "I apologize, there was an issue processing your request. Please try again.", lang

    async def process_query(user_text: str) -> str:
        answer, _ = await process_query_with_lang(user_text)
        return answer

    async def process_audio_query(transcript: str) -> tuple[str, str]:
        if not transcript or not transcript.strip():
            return "Please provide a message or question.", "en"

        lang = detect_language(transcript)

        if is_urdu_greeting(transcript):
            return "Welcome to Bank Islami. میں آپ کی کیسے مدد کر سکتا ہوں؟", "ur"
        if is_english_greeting(transcript):
            return "Welcome to Bank Islami. How may I help you?", "en"

        search_query = transcript
        if lang == "ur":
            search_query = await translate_to_english(transcript)
            print(f"Translated query: {search_query}")
        ...
        if lang == "ur":
            audio_system_prompt = (
                ...
                "Reply in simple, modern Pakistani Urdu.\n"
                "Keep sentences short (10-12 words max).\n"
                "Avoid heavy words like: استفسار، معزز، موصوف، رہنمائی، معاونت، تفصیلِ احوال، براہِ کرم، موصول، رقوم، منتقلی، ادائیگی.\n"
                "If a word is difficult, use a simpler Urdu word or an English word.\n"
                "Allowed English words: account, card, balance, statement, charges, limit, transaction, transfer."
            )
        else:
            audio_system_prompt = (
                f"{system_prompt}\n\n"
                "Use ONLY the context provided. If the answer is not in the context, say you don't know.\n"
                "Reply in clear, modern, conversational English.\n"
                "Use short sentences. Avoid formal banking jargon.\n"
                "Do not include Urdu words."
            )
    ...
    async def unified_message(
        ...
        try:
            if file:
                response_text, lang = await process_audio_query(message_text)
            else:
                response_text, lang = await process_query_with_lang(message_text)
        except Exception as e:
        ...
        try:
            ...
            if file:
                if lang == "ur":
                    tts_text = replace_hard_words(tts_text)
                audio_response = await synthesize_speech(tts_text, voice=select_voice(lang))
            else:
                audio_response = await synthesize_speech(tts_text, voice=select_voice(lang))
        except Exception as e:
            print(f"TTS error: {e}")
    ...
    async def audio_reply(file: UploadFile = File(...)) -> Response:
        ...
        answer, lang = await process_audio_query(transcript)
        tts_text = answer
        if lang == "ur":
            tts_text = replace_hard_words(tts_text)
        audio_out = await synthesize_speech(tts_text, voice=select_voice(lang))
    ...
    async def tts(text: str = Query(min_length=1)) -> Response:
        ...
        audio_out = await synthesize_speech(text, voice=select_voice(lang))
        return Response(content=audio_out, media_type=audio_content_type())
