...
def is_audio_greeting(text: str) -> bool:
    lowered = text.lower()
    if any(token in lowered for token in AUDIO_GREETINGS_ROMAN):
        return True
    if any(token in text for token in AUDIO_GREETINGS_URDU):
        return True
    return False
...
def create_app() -> FastAPI:
    ...
    async def process_audio_query(transcript: str) -> str:
        if not transcript or not transcript.strip():
            return "Please provide a message or question."

        if is_audio_greeting(transcript):
            return "Welcome to Bank Islami. میں آپ کی کیسے مدد کر سکتا ہوں؟"
        ...
        audio_system_prompt = (
            ...
            "Avoid heavy words like: استفسار، معزز، موصوف، رہنمائی، معاونت، تفصیلِ احوال، ازراہِ کرم، موصول، رقوم، منتقلی، ادائیگی.\n"
    ...
    async def unified_message(
        ...
        try:
            if file:
                response_text = await process_audio_query(message_text)
                lang = "ur"
            else:
        ...
        try:
            ...
            if file:
                ...
                audio_response = await synthesize_speech(tts_text, voice=select_voice("ur"))
            else:
    ...
    async def audio_reply(file: UploadFile = File(...)) -> Response:
        ...
        answer = await process_audio_query(transcript)
        tts_text = replace_hard_words(answer)
        audio_out = await synthesize_speech(tts_text, voice=select_voice("ur"))
